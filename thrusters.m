...
% list data files
data_dir = '/home/perlinm/lpf/workspace/code/data/';
...
if ~exist('set','var') || set > 7
  error('choose a valid data set');
elseif set <= 6
  load([data_dir,data_files{set}]);
else
  if set == 7
  ...
  load([file_dir,'Acc_1sctheta.mat']); tm1theta = objs;
  load([file_dir,'Acc_1sceta.mat']); tm1eta = objs;
  load([file_dir,'Acc_1scphi.mat']); tm1phi = objs;
  load([file_dir,'Acc_2scx.mat']); tm2x = objs;
  load([file_dir,'Acc_2scy.mat']); tm2y = objs;
  load([file_dir,'Acc_2scz.mat']); tm2z = objs;
  load([file_dir,'Acc_2sctheta.mat']); tm2theta = objs;
  load([file_dir,'Acc_2sceta.mat']); tm2eta = objs;
  load([file_dir,'Acc_2scphi.mat']); tm2phi = objs;
...
for s = 1:num_sets; accel_data(s).clearHistory(); end
for t = 1:num_thrusters; input_force(t).clearHistory(); end
...
for t = 1:num_thrusters
  ...
  % frequency of signal
  frequency(t) = max(input_force_spec(t).select(max_index),plist('axis','x'));
end; clear max_index;
time = toc; fprintf('%.0f sec\n',time);
...
for t = 1:num_thrusters
  notch_filter(t) = mfir(plist(...
    'name','notch filter',...
    'type','bandpass',...
    'fc',(frequency(t).y + [-input_bandwidth/2,input_bandwidth/2]),...
    'order',notch_order,...
    'win',notch_function,...
    'gain',1,...
...
for t = 1:num_thrusters
  ...
  for s = 1:num_sets
    ...
    accel(s,t) = signal_filter(accel_data(s),prefilter,notch_filter(t));
  end
  % filter input signals to match acceleration data
  input_wave(t) = signal_filter(input_force(t),prefilter,notch_filter(t));
end
time = toc; fprintf('    %.0f sec\n',time);
...
for t = 1:num_thrusters
  ...
  input_wave_nofilter(t).select(notch_order:len(input_wave_nofilter(t)));
  ...
  expected_shift(t) = angle(prefilter.resp(frequency(t).y)) ...
...
for s = 1:num_sets
  for t = 1:num_thrusters
    ...
    sig_transfer(s,t) = interp(transfer(s,t),plist('vertices',frequency(t).y));
...
for t = 1:num_thrusters
  for m = 1:num_masses
    ...
    accel_tm(t,m,:) = [accel(x_d(m),t),accel(y_d(m),t),accel(z_d(m),t)];
    angular_accel_tm(t,m,:) = ...
      [accel(theta_d(m),t),accel(eta_d(m),t),accel(phi_d(m),t)];
    % Euler acceleration
    accel_euler(t,m,:) = ao_cross(angular_accel_tm(t,m,:),r_tm(m,:));
    for i = 1:3
    ...
    [evecs(t,m,:,:),cov_diag(t,m,:,:)] = eigs(accel_com_cov(t,m).y);
    % thruster calibration, pitch, and x-y angle
    calib_all(t,m) = m_sc.y*sqrt(2*cov_diag(t,m,1,1))/input_mag(t).y;
    alpha_all(t,m) = atan2(-accel_com_dir(z_d(m),t)*abs(evecs(t,m,z_c,1)),...
                           sqrt(evecs(t,m,x_c,1)^2 + evecs(t,m,y_c,1)^2));
    beta_all(t,m) = atan2(-accel_com_dir(y_d(m),t)*abs(evecs(t,m,y_c,1)),...
                          -accel_com_dir(x_d(m),t)*abs(evecs(t,m,x_c,1)));
  end
  calib(t) = mean(calib_all(t,:));
  alpha(t) = mean(alpha_all(t,:));
...
for t = 1:num_thrusters
    for m = 1:num_masses
        ...
        com_resp_proj(t,m,:) = ...
          m_sc/input_mag(t)*in_basis(evecs(t,m,:,:),accel_com(t,m,:));
    end
end
time = toc; fprintf('%.0f sec\n',time);
